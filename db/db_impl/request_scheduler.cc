...
void RequestScheduler::DynamicLevelMoving(uint64_t &start_time){
  ...
  if(SPDK_TIME_DURATION(start_time, time_now, spdk_tsc_rate_) > 1000000){
    ...
    std::string time = ssdlogging::GetDayTime().c_str();
    double bd1 = spandb_controller_.GetLDBandwidth();
    double bd2 = spandb_controller_.GetDDBandwidth();
    double bd3 = spandb_controller_.GetLDBgBandwidth();
    // printf("Bandwidth: %s %.2lf %.2lf\n", time.c_str(), bd1, bd2);
    // printf("BgBanwidth: %s %.2lf\n", time.c_str(), bd2 + bd3);
