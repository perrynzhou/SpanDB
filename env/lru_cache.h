...
template <class TKey, class TValue, class THash>
ConcurrentLRUCache<TKey, TValue, THash>::ConcurrentLRUCache(size_t maxSize)
    : m_maxSize(maxSize),
      m_size(0),
...
template <class TKey, class TValue, class THash>
bool ConcurrentLRUCache<TKey, TValue, THash>::find(ConstAccessor& ac,
                                                   const TKey& key) {
  HashMapConstAccessor& hashAccessor = ac.m_hashAccessor;
  if (!m_map.find(hashAccessor, key)) {
    // __sync_fetch_and_add(&miss_num, 1);
    return false;
  }
  ...
  // __sync_fetch_and_add(&hit_num, 1);
  return true;
}

template <class TKey, class TValue, class THash>
bool ConcurrentLRUCache<TKey, TValue, THash>::insert(const TKey& key,
                                                     const TValue& value) {
...
template <class TKey, class TValue, class THash>
void ConcurrentLRUCache<TKey, TValue, THash>::clear() {
  // printf("delete_num: %lld\n", delete_num);
  // printf("insert_num: %lld\n", insert_num);
  // printf("evict_num: %lld\n", evict_num);
  // printf("hit: %lld, miss: %lld, hit ratio: %.3lf\n", hit_num, miss_num,
  // hit_num * 1.0 / (hit_num + miss_num));
  m_map.clear();
...
template <class TKey, class TValue, class THash>
void ConcurrentLRUCache<TKey, TValue, THash>::snapshotKeys(
...
template <class TKey, class TValue, class THash>
inline void ConcurrentLRUCache<TKey, TValue, THash>::delink(ListNode* node) {
...
template <class TKey, class TValue, class THash>
inline void ConcurrentLRUCache<TKey, TValue, THash>::pushFront(ListNode* node) {
...
template <class TKey, class TValue, class THash>
void ConcurrentLRUCache<TKey, TValue, THash>::evict() {
