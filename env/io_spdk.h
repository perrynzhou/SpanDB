...
#define SPDK_MEM_POOL_SIZE (20ull << 30)
...
typedef tstarling::ConcurrentLRUCache<int, std::shared_ptr<LRUEntry>> LRUCache;
...
struct BufferPool {
 ...
 public:
  BufferPool(int64_t size)
    ...
    for (int64_t i = 0; i < size_; i++) buffer_[i] = nullptr;
  }
  ~BufferPool() { delete buffer_; }
  void Put(char *value) {
  ...
  char *Get() {
    int64_t seq = next_read_.fetch_add(1) + 1;
    while (seq > last_wrote_.load()) {
    }
    char *value = buffer_[seq % size_];
  ...
  int64_t Size() {
    return last_wrote_.load(std::memory_order_relaxed) -
           last_read_.load(std::memory_order_relaxed);
  }
};
...
struct LRUEntry {
  ...
  ~LRUEntry() { spdk_mem_pool_->Put(data);}
};
...
static void Init(std::string pcie_addr, int logging_queue_num,
  ...
  lru_cache = new LRUCache(SPDK_MEM_POOL_ENTRY_NUM - 200);
 ...
 public:
  ...
  ~SpdkFile() {
    ...
    for (uint64_t i = 0; i < file_buffer_num_; i++) {
      if (file_buffer_[i]->data_ != nullptr) {
        ...
        lru_cache->insert(file_buffer_[i]->start_lpn_, value);
      }
  ...
  Status Read(uint64_t offset, size_t n, Slice *result, char *scratch) {
    ...
    while (n > 0) {
      ...
      LRUCache::ConstAccessor ac;
      char *data = nullptr;
      std::shared_ptr<LRUEntry> value;
      if (lru_cache->find(ac, lpn)) {
        std::shared_ptr<LRUEntry> entry = ac.get();
        data = entry->data;
#ifdef SPANDB_STAT
        hit = true;
#endif
      ...
      if (hit) {
        ...
        // read_miss_latency_.add(SPDK_TIME_DURATION(s_time, SPDK_TIME,
        // spdk_tsc_rate_));
      }
#endif
    }
...
__attribute__((unused)) static void ReadTest(SpdkFile *spdk_file, uint64_t num,
                                             uint64_t size, double *time,
                                             int index) {
  ssdlogging::SetAffinity("spdk test", index + 1);
  char *scratch = new char[size];
  Slice *result = new Slice(scratch, size);
  auto start = SPDK_TIME;
  for (uint64_t i = 0; i < num; i++) {
    uint64_t offset = Random::GetTLSInstance()->Next() % (5ull << 20);
    offset = offset * SPDK_PAGE_SIZE;
    spdk_file->Read(offset, size, result, scratch);
  }
  time[index] = SPDK_TIME_DURATION(start, SPDK_TIME, spdk_tsc_rate_);
}

__attribute__((unused)) static void TopsFSTest(Env *env) {
  uint64_t start_lpn = LO_FILE_START;
  uint64_t file_size = 20ull << 30;
  uint64_t request_size = 4096;
  int threads_num = 16;
  int num = 1000000;
  double *time = new double[threads_num];
  std::vector<std::thread> threads;
  for (int i = 0; i < threads_num; i++) {
    std::string file_name = "test_file-" + std::to_string(i);
    uint64_t end_lpn = start_lpn + file_size / SPDK_PAGE_SIZE - 1;
    SPDKFileMeta *meta = new SPDKFileMeta(file_name, start_lpn, end_lpn, false);
    SpdkFile *spdk_file = new SpdkFile(env, file_name, meta, file_size, false);
    printf("start: %ld, end: %ld\n", start_lpn, end_lpn);
    threads.emplace_back(ReadTest, spdk_file, num, request_size, time, i);
    start_lpn = end_lpn + 1;
  }
  for (auto &t : threads) {
    t.join();
